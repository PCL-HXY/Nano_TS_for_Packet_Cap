...
extern uint64_t dsp_timestamp_rx_dynflag;
extern int dsp_timestamp_dynfield_offset;

static inline rte_mbuf_timestamp_t *
dsp_timestamp_dynfield(struct rte_mbuf *mbuf)
{
	return RTE_MBUF_DYNFIELD(mbuf,
		dsp_timestamp_dynfield_offset, rte_mbuf_timestamp_t *);
}
...
rematch:
		...
		// timestamp
		if (dsp_timestamp_dynfield_offset >= 0) {
			rte_mbuf_timestamp_t timestamp;
			/* nanoseconds */
			timestamp = RTE_STATIC_BSWAP64(*((uint64_t *)(usr_rbuffer + usr_rdp)));
			*dsp_timestamp_dynfield(mbuf) = timestamp;
			mbuf->ol_flags |= dsp_timestamp_rx_dynflag;
		}
